...
public class YibaoController {
    ...
    */
    ...
    public R update(@RequestBody YibaoEntity yibao, HttpServletRequest request){
        logger.debug("update方法:,,Controller:{},,yibao:{}",this.getClass().getName(),yibao.toString());

        Wrapper<YibaoEntity> queryWrapper = new EntityWrapper<YibaoEntity>()
            .notIn("id",yibao.getId())
            .andNew()
            .eq("yonghu_id", yibao.getYonghuId())
            ;
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids){
        ...
        yibaoService.deleteBatchIds(Arrays.asList(ids));
        return R.ok();
    }
...
//                            yibaoEntity.setYibaoUuidNumber(data.get(0));                    //医保编号 要改的
//                            yibaoEntity.setYibaoTypes(Integer.valueOf(data.get(0)));   //医保类型 要改的
//                            yibaoEntity.setYibaoTime(sdf.parse(data.get(0)));          //医保到期日期 要改的
...
//                            yibaoEntity.setCreateTime(date);//时间
                        ...
                        //查询是否重复
                         //医保编号
                        List<YibaoEntity> yibaoEntities_yibaoUuidNumber = yibaoService.selectList(new EntityWrapper<YibaoEntity>().in("yibao_uuid_number", seachFields.get("yibaoUuidNumber")));
                        if(yibaoEntities_yibaoUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [医保编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
