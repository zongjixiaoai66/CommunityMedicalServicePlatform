    ...
    */
    ...
    public R save(@RequestBody YaopinEntity yaopin, HttpServletRequest request){
        ...
        Wrapper<YaopinEntity> queryWrapper = new EntityWrapper<YaopinEntity>()
            ...
            .eq("yaopin_guige", yaopin.getYaopinGuige())
            .eq("yaopin_kucun_number", yaopin.getYaopinKucunNumber())
            .eq("shengchanchangshang", yaopin.getShengchanchangshang())
            .eq("jixing_types", yaopin.getJixingTypes())
            .eq("yongfayongliang", yaopin.getYongfayongliang())
            ;
...
//                            yaopinEntity.setYaopinUuidNumber(data.get(0));                    //药物编号 要改的
//                            yaopinEntity.setYaopinName(data.get(0));                    //药物名称 要改的
//                            yaopinEntity.setYaopinTypes(Integer.valueOf(data.get(0)));   //药物类型 要改的
//                            yaopinEntity.setYaopinGuige(data.get(0));                    //产品规格 要改的
//                            yaopinEntity.setYaopinKucunNumber(Integer.valueOf(data.get(0)));   //药物库存数量 要改的
//                            yaopinEntity.setShengchanchangshang(data.get(0));                    //生产厂商 要改的
//                            yaopinEntity.setJixingTypes(Integer.valueOf(data.get(0)));   //药物剂型 要改的
//                            yaopinEntity.setYongfayongliang(data.get(0));                    //用法用量 要改的
//                            yaopinEntity.setGongxiaoContent("");//详情和图片
//                            yaopinEntity.setBuliangContent("");//详情和图片
...
//                            yaopinEntity.setCreateTime(date);//时间
                        ...
                        if(yaopinEntities_yaopinUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [药物编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
